...
{
    public class RdpPacket : MemoryStream
    {
        ...
        private NetworkSocket m_Socket = null;
        ...
        private void ReadFromSocket(int lengthRequired)
        {
            ...
            int count = m_Socket.Receive(m_Buffer, m_Buffer.Length);
            if (count <= 0)
            {
                throw new IOException("RdpPacket - Overrun!");
            }
        ...
        public string ReadUnicodeString()
        {
            ...
            int num = this.getLittleEndian32() - 2;
            for (int i = 0; i < num; i += 2)
            {
                int num3 = this.getLittleEndian16();
                builder.Append((char) num3);
            }
            this.getLittleEndian16();
